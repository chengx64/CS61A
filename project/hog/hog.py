...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        if tmp == 1:
            has_one = 1
        sum += tmp
    return 1 if has_one == 1 else sum
    # END PROBLEM 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    roll_dice_num = strategy(0, 0)
    for player_score in range(goal):
        for opponent_score in range(goal):
            if roll_dice_num != strategy(player_score, opponent_score):
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    i = 1
    while i <= 10:
        tmp_current = make_averaged(roll_dice, samples_count)(i, dice)
