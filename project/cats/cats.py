...
def pick(paragraphs, select, k):
    ...
    if len(paragraphs) == 0:
        return ''
    elif k == 0 and select(paragraphs[0]):
        return paragraphs[0]
    elif k == 0 and not select(paragraphs[0]):
        return pick(paragraphs[1:], select, 0)
    elif select(paragraphs[0]):
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        return typed_word
    lst_diff = [diff_function(typed_word, i, limit) for i in word_list]
    if min(lst_diff) > limit:
        return typed_word
    else:
...
def feline_fixes(typed, source, limit):
    ...
    1
    >>> feline_fixes("range", "rungs", big_limit)  # Substitute: a -> u, e -> subject
    2
    >>> feline_fixes("pill", "pillage", big_limit) # Don't substitute anything, length difference of 3.
    3
    >>> feline_fixes("roses", "arose", big_limit)  # Substitute: r -> a, o -> r, subject -> o, e -> subject, subject -> e
    5
    >>> feline_fixes("rose", "hello", big_limit)   # Substitute: r->h, o->e, subject->l, e->l, length difference of 1.
    5
    """
    # BEGIN PROBLEM 6
    if typed == source:
...
def fastest_words(match):
    ...
    for i in word_indices:
        ...
        for k in player_indices:
            time.append(get_all_times(match)[k][i])
        min_time = min(time)
        for t in player_indices:
            if get_all_times(match)[t][i] == min_time:
                result[t] = result[t] + [get_word(match, i)]
                break
    return result
    # END PROBLEM 10
